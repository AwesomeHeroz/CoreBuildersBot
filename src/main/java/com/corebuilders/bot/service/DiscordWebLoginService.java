...
import static com.corebuilders.bot.service.DiscordWebLoginChallengeRepository.*;

/** Framework-free application service for website login verified through the Discord bot. */
public final class DiscordWebLoginService {
    private static final char[] CODE_ALPHABET = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789".toCharArray();
    private static final int CODE_LENGTH = 8;
    private static final int MAX_CREATE_ATTEMPTS = 10;

    private final DiscordWebLoginChallengeRepository repository;
    private final Duration challengeLifetime;
    private final Clock clock;
    private final SecureRandom random;
    private final byte[] hashPepper;

    public DiscordWebLoginService(DiscordWebLoginChallengeRepository repository, Duration challengeLifetime) {
        this(repository, challengeLifetime, Clock.systemUTC(), new SecureRandom(), randomPepper());
    }

    DiscordWebLoginService(
            DiscordWebLoginChallengeRepository repository,
        ...
        if (this.hashPepper.length < 32) {
            throw new IllegalArgumentException("Login hash pepper must be at least 32 bytes.");
        }
    }

    public static DiscordWebLoginService disabled() {
        DiscordWebLoginChallengeRepository unavailable = new DiscordWebLoginChallengeRepository() {
            @Override public void create(NewChallenge challenge) {
                throw new IllegalStateException("Discord bot website login is not configured.");
            }
            ...
            @Override public CompletionResult complete(String browserTokenHash, Instant now, boolean consume) {
                return new CompletionResult(CompletionStatus.INVALID, null, null, null);
            }
        };
        return new DiscordWebLoginService(unavailable, Duration.ofMinutes(10));
    }
    ...
    public StartChallenge createChallenge() {
        ...
        for (int attempt = 0; attempt < MAX_CREATE_ATTEMPTS; attempt++) {
            ...
            try {
                repository.create(new NewChallenge(
                        UUID.randomUUID(),
                        hash(browserToken),
                        hash(compactCode),
                        expiresAt,
                ...
                return new StartChallenge(
                        browserToken,
                        displayCode,
                        "/core web-login code:" + displayCode,
                        expiresAt
                );
        ...
        throw new IllegalStateException("Could not generate a unique Discord login challenge. Try again.", lastCollision);
    }

    public VerificationResult verifyFromDiscord(
            String rawCode,
            String discordUserId,
            String discordUsername,
            String discordAvatarUrl
        ...
        return repository.verify(
                hash(normalizeCode(rawCode)),
                requireDiscordUserId(discordUserId),
                safe(discordUsername, 100, "Unknown"),
                safe(discordAvatarUrl, 1000, ""),
                clock.instant()
        );
    }
    ...
    static String normalizeCode(String value) {
        if (value == null || value.isBlank()) {
            throw new IllegalArgumentException("A website login code is required.");
        }
        ...
        for (int i = 0; i < normalized.length(); i++) {
            ...
            for (char allowed : CODE_ALPHABET) {
                if (normalized.charAt(i) == allowed) {
                    accepted = true;
                    break;
                }
            }
            if (!accepted) {
                throw new IllegalArgumentException("The website login code contains an invalid character.");
            }
        }
        return normalized;
    }
    ...
    private static String requireBrowserToken(String value) {
        if (value == null || value.isBlank()) {
            throw new IllegalArgumentException("A login challenge token is required.");
        }
        String trimmed = value.trim();
        if (trimmed.length() > 200) {
            throw new IllegalArgumentException("The login challenge token is invalid.");
        }
        return trimmed;
    }

    private static String requireDiscordUserId(String value) {
        if (value == null || !value.matches("[0-9]{5,32}")) {
            throw new IllegalArgumentException("A valid Discord user ID is required.");
        }
        return value;
    }
