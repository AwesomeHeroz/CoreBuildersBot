...
public final class WebLoginService {
    ...
    public WebLoginService(WebLoginChallengeRepository repository, Duration challengeLifetime) {
        this(repository, challengeLifetime, Clock.systemUTC(), new SecureRandom());
    }

    WebLoginService(
            WebLoginChallengeRepository repository,
    ...
    public VerificationResult verifyFromGame(String rawCode, UUID minecraftUuid, String minecraftName) {
        ...
        return repository.verify(
                ...
                minecraftUuid,
                safeMinecraftName(minecraftName),
                clock.instant()
        );
    }

    public CompletionResult complete(String rawBrowserToken) {
        String token = requireBrowserToken(rawBrowserToken);
        return repository.complete(sha256(token), clock.instant());
    }
    ...
    static String normalizeCode(String value) {
        ...
        for (int i = 0; i < normalized.length(); i++) {
            char valueAt = normalized.charAt(i);
